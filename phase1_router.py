...
def parent_link(payload: Dict[str, Any], u: Dict[str, Any] = Depends(require_role("parent"))):
    """Link the logged-in parent to a student via a one-time code.

    This is the primary production flow for KnowEasy:
    - Parent logs in with their own email OTP (role=parent)
    - Parent enters the child's one-time code
    - We create a durable parent↔student link (DB/Redis)

    Returns {ok:true, student_user_id, message} on success.
    Returns 400 with detail on invalid/expired/used codes.
    """
    code = (payload.get("code") or "").strip()
    if not code:
        raise HTTPException(status_code=400, detail="Invalid or missing code")

    ok, msg, sid = phase1_store.link_parent_with_code(parent_user_id=_uid(u), code=code)
    if not ok or not sid:
        raise HTTPException(status_code=400, detail=msg or "Could not link.")
    return {"ok": True, "student_user_id": int(sid), "message": msg or "Linked successfully."}


@router.post("/parent/link-public")
def parent_link_public(payload: Dict[str, Any]):
    """Optional public link flow (no parent account).

    This keeps backward compatibility with the earlier Phase-1 prototype that
    issued a long-lived parent session token without creating a parent user.
    The app currently uses the authenticated /parent/link flow.
    """
    code = (payload.get("code") or "").strip()
    if not code:
        raise HTTPException(status_code=400, detail="Invalid or missing code")

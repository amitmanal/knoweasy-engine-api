...
def study_assets(
    ...
    expires: int | None = Query(None, ge=30, le=3600),
    debug: int = Query(0),
):
    """Return active assets for a given content_id.

    This is the canonical replacement for Hostinger /content/... assumptions.
    """
    ...
    base = (os.getenv("CONTENT_BASE_URL") or "https://knoweasylearning.com/content").rstrip("/")
    ...
    for a in assets:
        ...
        if storage == "r2" and object_key:
            try:
                ...
                url = None
        elif storage == "hostinger":
            # url can be a full URL OR a relative path under /content
            if url and str(url).startswith("http"):
                pass
            elif url:
                url = f"{base}/{str(url).lstrip('/')}"
            elif object_key:
                url = f"{base}/{str(object_key).lstrip('/')}"
    ...
    out = {"ok": True, "content_id": cid, "assets": by_type}
    if debug:
        out["debug"] = {"count": len(assets), "types": list(by_type.keys())}
    return out

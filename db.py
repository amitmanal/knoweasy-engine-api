...
from shared_engine import get_engine, db_health  # noqa: F401
...
def _get_engine():
    """Use the shared engine instead of creating a separate pool."""
    return get_engine()
...
def db_init() -> Dict[str, Any]:
    ...
    try:
        ...
        # FIX: Add critical missing indexes for scale
        indexes = [
            "CREATE INDEX IF NOT EXISTS idx_ask_logs_created ON ask_logs(created_at DESC);",
            ...
            "CREATE INDEX IF NOT EXISTS idx_learning_memory_user ON learning_memory_cards(user_id);",
        ]
...
def db_cleanup_expired():
    """Delete expired OTPs and sessions. Call from background task."""
    engine = _get_engine()
    if engine is None:
        return {"ok": False, "reason": "DB not available"}
    try:
        with engine.begin() as conn:
            r1 = conn.execute(text("DELETE FROM otp_codes WHERE expires_at < NOW()"))
            r2 = conn.execute(text("DELETE FROM sessions WHERE expires_at < NOW()"))
            deleted_otps = r1.rowcount if r1 else 0
            deleted_sessions = r2.rowcount if r2 else 0
        logger.info("Cleanup: deleted %d expired OTPs, %d expired sessions", deleted_otps, deleted_sessions)
        return {"ok": True, "deleted_otps": deleted_otps, "deleted_sessions": deleted_sessions}
    except Exception as e:
        logger.exception("db_cleanup_expired failed")

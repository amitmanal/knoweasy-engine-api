...
def ensure_tables() -> None:
    ...
    try:
        with engine.begin() as conn:
            ...
            # Canonical content asset registry (R2/Hostinger). This is the production replacement
            # for Hostinger /content/* assumptions.
            conn.execute(_t("""
            ...
            """))
            conn.execute(_t("""
                CREATE INDEX IF NOT EXISTS ix_content_assets_content
                ON content_assets (content_id);
            """))
            conn.execute(_t("""
                CREATE INDEX IF NOT EXISTS ix_content_assets_active
...
def seed_syllabus_from_packaged_files(seed_dir: Path, reset: bool = False) -> Dict[str, Any]:
    ...
    def upsert_many(rows: List[Dict[str, Any]]):
        ...
        # We want syllabus_map to be canonical and NEVER be blocked by legacy constraints.
        # So we commit syllabus_map upserts even if syllabus_chapters fails.
        for r in rows:
            try:
                content_id = r.get("content_id", None)
                # 1) Canonical upsert (its own transaction)
                with engine.begin() as conn:
                    conn.execute(
                        _t(
                            """
                ...
                # 2) Legacy upsert (best-effort; never blocks syllabus_map)
                try:
                    with engine.begin() as conn:
                        conn.execute(
                            _t(
                    ...
                    # Ignore legacy failures (FK constraints, etc.)
                    if first_error is None:
                        first_error = f"LEGACY_SEED_WARNING: {e2}"
...
def set_content_asset(
    ...
    """Insert a content asset and optionally make it the active one for that asset_type."""
    ensure_tables()
    e = get_engine_safe()
    if not e:
    ...
    try:
        with e.begin() as conn:
            if activate:
                conn.execute(_t("""
                    ...
                    WHERE content_id=:cid AND asset_type=:at AND is_active=TRUE
                """), {"cid": cid, "at": at})

            r = conn.execute(_t("""
                INSERT INTO content_assets(content_id, asset_type, storage, object_key, url, mime_type, meta_json, is_active)
                VALUES (:cid,:at,:st,:okey,:url,:mime,:meta,:active)
                RETURNING id
            """), {
                "cid": cid,
                "at": at,
        ...
        logger.warning(f"set_content_asset failed: {e}")
        return {"ok": False, "error": "DB_WRITE_FAILED", "message": str(e)}


def get_active_content_assets(content_id: str) -> List[Dict[str, Any]]:
    """Return active assets for a content_id."""
    ensure_tables()
    e = get_engine_safe()
    if not e:
        return []
    ...
    try:
        with e.connect() as conn:
            rs = conn.execute(_t("""
                SELECT id, asset_type, storage, object_key, url, mime_type, meta_json
                FROM content_assets
                WHERE content_id=:cid AND is_active=TRUE
                ORDER BY created_at DESC
            """), {"cid": cid}).fetchall()
        ...
        logger.warning(f"get_active_content_assets failed: {e}")
        return []